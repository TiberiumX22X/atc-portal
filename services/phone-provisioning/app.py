...
def _migrate_add_column_if_missing(db_path, table, column, coltype):
    """Лёгкая миграция для серверов, где schema.sql уже применялась ДО того,
    как в неё добавили эту колонку (Этап 5 добавил original_content к уже
    существовавшей с Этапа 1 таблице phone_vendors) — CREATE TABLE IF NOT
    EXISTS такую колонку старой установке не добавит, а INSERT из schema.sql
    в init_db() ниже сразу упадёт на "no such column", если её нет. Поэтому
    вызывается ДО init_db(). PRAGMA table_info на несуществующей таблице
    просто возвращает пустой список (не бросает исключение) — значит,
    на действительно свежей БД (таблицы ещё нет вообще) эта функция ничего
    не делает и не мешает CREATE TABLE из init_db() создать таблицу сразу
    с нужной колонкой."""
...
def get_models_by_vendor(conn):
    """Модели, сгруппированные по вендору — для JS-фильтруемого select'а
    вендор+модель в форме устройства и на странице прошивок. Grandstream
    берётся из DROPDOWN_MODELS (свой путь, без БД); Fanvil/Yealink и
    будущие вендоры — из phone_vendors/phone_models."""
    result = {"Grandstream": DROPDOWN_MODELS}
    vendors = conn.execute("SELECT id, name FROM phone_vendors ORDER BY name").fetchall()
    for v in vendors:
        rows = conn.execute(
            "SELECT model_key FROM phone_models WHERE vendor_id = ? ORDER BY display_name", (v["id"],)
        ).fetchall()
        result[v["name"]] = [r["model_key"] for r in rows]
    return result
...
# ---------- Мульти-вендорный провижининг (Fanvil, Yealink и т.п.) ----------
# Отдельная ветка поверх MODEL_FAMILIES выше — существующий Grandstream-путь
# (сам словарь, canonical_model/model_variants, шаблоны .xml.j2 в
# templates/) этим кодом не затрагивается ни в одной строке. Шаблоны и
# список моделей для новых вендоров живут в БД — phone_vendors/
# phone_models/phone_vendor_settings (см. schema.sql) — а не в файлах.
from jinja2.sandbox import SandboxedEnvironment


def get_vendor_settings(conn, vendor_id):
    rows = conn.execute(
        "SELECT key, value FROM phone_vendor_settings WHERE vendor_id = ?", (vendor_id,)
    ).fetchall()
    return {r["key"]: r["value"] for r in rows}


def _sanitize_cfg_value(value):
    """Формат cfg (Yealink: plain key=value) не экранируется никак сам по
    себе — перевод строки в значении (например, если в имя/метку устройства
    случайно попал Enter при вставке из другого источника) позволил бы
    дописать в конфиг произвольную дополнительную строку вида
    account.1.password=... . Для xml-формата от этого защищает автоэкранирование
    Jinja2 (autoescape=True ниже), для cfg — вырезаем переводы строк вручную."""
    if value is None:
        return ""
    return str(value).replace("\r", "").replace("\n", " ")


def render_multivendor_config(conn, device, mac):
    """Пытается собрать конфиг для вендора, отличного от Grandstream.
    Возвращает (содержимое, mimetype) или None, если модель/вендор не
    зарегистрированы в БД — в этом случае вызывающий код возвращает 404
    точно так же, как раньше для \"Unknown model\" у Grandstream."""
    vendor_id = (device["manufacturer"] or "").strip().lower()
    if vendor_id in ("", "grandstream"):
        return None  # Grandstream всегда обрабатывается старым путём, сюда не заходит

    vendor = conn.execute("SELECT * FROM phone_vendors WHERE id = ?", (vendor_id,)).fetchone()
    if not vendor:
        return None

    model_row = conn.execute(
        "SELECT * FROM phone_models WHERE vendor_id = ? AND model_key = ?",
        (vendor_id, device["model"]),
    ).fetchone()
    if not model_row:
        return None

    s = get_vendor_settings(conn, vendor_id)
    fmt = vendor["template_format"]  # 'xml' | 'cfg'

    # Прошивка: та же общая таблица firmware, что и у Grandstream (ключ —
    # просто строка модели; коллизий с Grandstream не ожидается, у них
    # разные схемы именования моделей — GXP2170 vs X4U/SIP-T46S и т.п.)
    fw_row = conn.execute(
        "SELECT filename FROM firmware WHERE model = ? AND enabled = 1", (device["model"],)
    ).fetchone()
    firmware_file = fw_row["filename"] if fw_row else ""

    # SandboxedEnvironment — шаблон вендора хранится в БД и редактируется
    # через панель (вкладка «Шаблоны», см. следующий этап), т.е. это по сути
    # пользовательский код; без песочницы через него был бы доступ к
    # файловой системе сервера через встроенные атрибуты Python-объектов.
    env = SandboxedEnvironment(autoescape=(fmt == "xml"))
    template = env.from_string(vendor["template_content"])

    if fmt == "xml":
        rendered = template.render(mac=mac, device=device, s=s, model=device["model"], firmware_file=firmware_file)
        mimetype = "text/xml"
    else:
        safe_device = {k: _sanitize_cfg_value(device[k]) for k in device.keys()}
        safe_s = {k: _sanitize_cfg_value(v) for k, v in s.items()}
        rendered = template.render(mac=mac, device=safe_device, s=safe_s, model=device["model"], firmware_file=_sanitize_cfg_value(firmware_file))
        mimetype = "text/plain"
...
def _update_last_seen(conn, device_id):
    ...
    Используется и Grandstream-, и мульти-вендорным путём провижининга."""
    ...
    conn.execute(
        ...
        (phone_ip, datetime.now().isoformat(timespec="seconds"), device_id),
    )
    conn.commit()
...
def provision_config(mac):
    ...
    if not family:
        # Не Grandstream (или неизвестная модель Grandstream) — пробуем
        # мульти-вендорный путь (Fanvil и т.п. тоже иногда запрашивают
        # cfg<mac>.xml — тот же URL-паттерн, что и у Grandstream).
        multivendor = render_multivendor_config(conn, device, mac)
        if multivendor:
            content, mimetype = multivendor
            _update_last_seen(conn, device["id"])
            conn.close()
...
@app.route("/<mac>.cfg")
def provision_config_cfg(mac):
    """Второй эндпоинт провижининга — для вендоров, которые запрашивают
    конфиг как <MAC>.cfg без префикса (Yealink; в отличие от Grandstream/
    Fanvil, использующих cfg<MAC>.xml, см. provision_config выше).
    Grandstream сюда никогда не попадает — этот роут сразу уходит в
    мульти-вендорный путь, MODEL_FAMILIES здесь не участвует вообще."""
    mac = normalize_mac(mac)
    if not MAC_RE.match(mac):
        return Response(status=404)  # не похоже на MAC — не наш файл, пусть достаётся catch-all/404
...
def device_new():
    ...
    if request.method == "POST":
        ...
        conn.close()
        return redirect(url_for("devices_list"))
    models_by_vendor = get_models_by_vendor(conn)
    conn.close()
    return render_template("device_form.html", device=None, models=DROPDOWN_MODELS, models_by_vendor=models_by_vendor)
...
def device_edit(device_id):
    ...
    models_by_vendor = get_models_by_vendor(conn)
    conn.close()
    return render_template("device_form.html", device=device, blf_keys=blf_keys, models=DROPDOWN_MODELS, models_by_vendor=models_by_vendor)
...
def firmware_page():
    ...
    return render_template("firmware.html", firmware=firmware_with_size, models=DROPDOWN_MODELS, models_by_vendor=models_by_vendor)
...
# ---------- Шаблоны мульти-вендорного провижининга (Fanvil, Yealink и т.п.) ----------
# Grandstream сюда не входит — у него свой путь через MODEL_FAMILIES и файлы
# .xml.j2 (см. выше), эта вкладка их не показывает и не может редактировать.

GITHUB_REPO_URL = "https://github.com/TiberiumX22X/atc-portal"
...
# Что подставлять в шаблон при тестовом рендере (тот самый "тестовый рендер
# на фиктивных данных" — ловит синтаксически битый Jinja2/XML до того, как
# правки вообще можно будет сохранить и они уйдут на живой телефон).
_FAKE_DEVICE_FOR_PREVIEW = {
    "extension": "7000", "label": "Тестовый телефон", "name": "Тест",
    "sip_password": "PreviewPass123", "active": 1,
}


def _preview_render(vendor_row, model_key, content):
    """Пробует отрендерить шаблон на фиктивных данных. Возвращает
    (rendered_or_None, error_or_None) — не бросает исключений наружу,
    вызывающий код сам решает, что делать с ошибкой (не сохранять правку)."""
    s = {
        "sip_server": "192.168.1.100", "sip_port": "5060",
        "ntp_server": "pool.ntp.org", "timezone": "auto",
        "language": "ru", "admin_password": "PreviewAdminPass",
        "firmware_server": "192.168.1.100:8090/firmware",
    }
    fmt = vendor_row["template_format"]
    env = SandboxedEnvironment(autoescape=(fmt == "xml"))
    try:
        template = env.from_string(content)
        rendered = template.render(
            mac="001122334455", device=_FAKE_DEVICE_FOR_PREVIEW, s=s, model=model_key or "",
            firmware_file="preview-firmware.bin",
        )
    except Exception as e:
    ...
    if fmt == "xml":
        ...
        try:
            ...
            return rendered, f"Отрендерилось, но это невалидный XML: {e}"
...
def templates_page():
    ...
    conn = get_db()
    vendors = conn.execute("SELECT * FROM phone_vendors ORDER BY name").fetchall()
    models_by_vendor = {}
    issue_urls = {}
    for v in vendors:
        rows = conn.execute(
            "SELECT * FROM phone_models WHERE vendor_id = ? ORDER BY display_name", (v["id"],)
        ).fetchall()
        models_by_vendor[v["id"]] = rows
        for m in rows:
            qs = urlencode({
                "title": f"Баг: {m['display_name']} — провижининг",
                "labels": "bug",
                "body": f"Модель: {m['display_name']}\nВендор: {v['name']}\n\nЧто произошло:\n",
            })
            issue_urls[m["id"]] = f"{GITHUB_REPO_URL}/issues/new?{qs}"
    conn.close()
    return render_template(
        "templates_list.html",
        vendors=vendors,
...
@app.route("/admin/templates/<vendor_id>/edit", methods=["GET", "POST"])
@login_required
def template_edit(vendor_id):
    conn = get_db()
    vendor = conn.execute("SELECT * FROM phone_vendors WHERE id = ?", (vendor_id,)).fetchone()
    if not vendor:
        conn.close()
        return Response("Unknown vendor", status=404)

    preview_result, preview_error = None, None
    ...
    if request.method == "POST":
        ...
        if action == "restore":
            if vendor["original_content"]:
                new_content = vendor["original_content"]
            else:
                flash("Оригинал недоступен (шаблон был добавлен уже как custom, без пристины) — восстанавливать нечего", "error")
                conn.close()
                return redirect(url_for("template_edit", vendor_id=vendor_id))

        preview_result, preview_error = _preview_render(vendor, None, new_content)

        if action in ("save", "restore") and not preview_error:
            new_source = "built_in" if action == "restore" else "custom"
            conn.execute(
                "UPDATE phone_vendors SET template_content = ?, source = ?, updated_at = datetime('now') WHERE id = ?",
                (new_content, new_source, vendor_id),
            )
            conn.commit()
            conn.close()
            flash("Шаблон сохранён" if action == "save" else "Восстановлен оригинал", "ok")
            return redirect(url_for("template_edit", vendor_id=vendor_id))

        if preview_error:
            flash(f"Не сохранено — ошибка рендера: {preview_error}", "error")
        # перечитываем vendor с учётом того, что именно ввёл пользователь
        # (не то, что уже в БД), чтобы форма не потеряла правки при ошибке
        vendor = dict(vendor)
        vendor["template_content"] = new_content
...
@app.route("/admin/templates/<vendor_id>/models/<int:model_id>/status", methods=["POST"])
@login_required
def model_status_update(vendor_id, model_id):
    status = request.form.get("status")
    note = request.form.get("status_note", "").strip()
    if status not in STATUS_LABELS:
        return Response("Bad status", status=400)
    conn = get_db()
    verified_at = "datetime('now')" if status == "verified" else "NULL"
    conn.execute(
        f"UPDATE phone_models SET status = ?, status_note = ?, verified_at = {verified_at} "
        f"WHERE id = ? AND vendor_id = ?",
        (status, note or None, model_id, vendor_id),
    )
    conn.commit()
    conn.close()
    flash("Статус модели обновлён", "ok")
...
# Миграция — ДО init_db(): у init_db() есть INSERT с явным списком колонок
# (включая original_content), который упадёт на существующей старой таблице,
# если колонку не добавить заранее (см. docstring миграции выше).
_migrate_add_column_if_missing(DB_PATH, "phone_vendors", "original_content", "TEXT")
init_db()
