...
@auth.login_required
def portal_home():
    alerts = db.get_active_alerts() if session.get("role") == "admin" else []
    return render_template("portal.html", panels=config.PANELS, alerts=alerts)
...
def dismiss_alert():
    ...
    try:
        alerts_dismiss.dismiss(key, node)
    except alerts_dismiss.DismissError as exc:
        flash(str(exc), "error")
    return redirect(url_for("portal_home"))


@app.route("/alerts/dismiss-all", methods=["POST"])
@auth.admin_required
def dismiss_all_alerts():
    auth.check_csrf()
    ok, failed = alerts_dismiss.dismiss_all()
    if failed:
        flash(f"Скрыто уведомлений: {ok}, не удалось: {failed} (нет SSH-доступа до соседнего узла?)",
              "warning" if ok else "error")
    else:
        flash(f"Скрыто уведомлений: {ok}.", "success")
    return redirect(url_for("portal_home"))
