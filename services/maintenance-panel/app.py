...
def backup_restore():
    ...
    try:
        ...
        restore_force = request.form.get("force_restore") == "1"
        restored, restarted, db_dump_present = backup.restore_backup_archive(tmp_path, force=restore_force)
        msg = (
...
# ---------------------------------------------------------------------------
# fail2ban — исключения (ignoreip). Полезно, если jail 'recidive' по ошибке
# забанит IP другой АТС/шлюза, с которым у вас транк — звонки перестают
# ходить без видимой связи с настройками FreePBX/Asterisk, см.
# fail2ban_ignoreip.py.
# ---------------------------------------------------------------------------

@app.route("/fail2ban-ignoreip", methods=["GET", "POST"])
@auth.admin_required
def fail2ban_ignoreip_page():
    if request.method == "POST":
        auth.check_csrf()
        action = request.form.get("action")
        subnet = request.form.get("subnet", "")
        try:
            if action == "add":
                peer_warning = fail2ban_ignoreip.add_ignoreip(subnet)
                if peer_warning:
                    flash(f"{subnet} добавлен локально, но {peer_warning}", "warning")
                else:
                    flash(f"{subnet} добавлен в исключения fail2ban (ignoreip) на обоих узлах.", "success")
            elif action == "remove":
                peer_warning = fail2ban_ignoreip.remove_ignoreip(subnet)
                if peer_warning:
                    flash(f"{subnet} убран локально, но {peer_warning}", "warning")
                else:
                    flash(f"{subnet} убран из исключений fail2ban на обоих узлах.", "success")
        except fail2ban_ignoreip.ValidationError as exc:
            flash(str(exc), "error")
        return redirect(url_for("fail2ban_ignoreip_page"))

    entries, error = fail2ban_ignoreip.get_ignoreip_list()
    return render_template("fail2ban_ignoreip.html", entries=entries, error=error)
