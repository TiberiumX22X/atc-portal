...
def _read_ha_role():
    """ROLE из /etc/atc-portal-ha.conf, если HA на этом сервере настроен —
    иначе None (не HA-инсталляция, проверка ниже просто не применяется)."""
...
def restore_backup_archive(uploaded_file_path, force=False):
    ...
    В HA-кластере ПОЧТИ ВСЕ файлы из BACKUP_PATHS (все базы панелей в
    /opt/*, /etc/asterisk/manager_custom.conf, /etc/nginx/conf.d/
    portal.conf) каждые 3 минуты ПЕРЕЗАПИСЫВАЮТСЯ sync_panels.sh —
    но только В ОДНУ СТОРОНУ, с master на backup (см. lib/ha.sh /
    ha_apply.sh). Если восстановить архив на backup-узле — все
    восстановленные файлы молча затрутся текущей копией с master в
    течение ближайших 3 минут, без единого предупреждения. Поэтому
    восстанавливать нужно ТОЛЬКО на текущем master — оттуда изменения
    сами разъедутся на backup обычной синхронизацией. Единственный
    файл, которого это не касается — fail2ban/jail.local (не входит в
    sync_panels.sh), но раз архив восстанавливается одним действием
    целиком, проще требовать master для всей операции.
    ...
    """
    role = _read_ha_role()
    if role == "backup" and not force:
        raise RestoreError(
            "Этот сервер сейчас в роли BACKUP в HA-кластере — восстановленные файлы "
            "будут молча затёрты обратно текущей копией с master в течение "
            "ближайших 3 минут (обычная синхронизация панелей). Выполните "
            "восстановление на текущем MASTER — оттуда изменения сами разъедутся "
            "на backup."
        )
